...
if result:
    ...
    if result.get("guardrail_blocked", False):
        ...
        st.error("🚫 Travel Request Blocked")

        st.subheader("Request Not Supported")

        st.warning(
            result.get(
                "supervisor_reasoning",
                "This request is not related to travel planning."
            )
        )

        st.info(
            "This application is designed for travel planning. "
            "Please ask something related to flights, hotels, "
            "weather, budget, destinations, or itineraries."
        )

        # Stop rendering the normal travel UI
        st.stop()
...
if (
    final_result
    and not final_result.get("guardrail_blocked", False)
