...
async def tavily_search(query: str):
    return await call_tool("tavily_tavily_search", {"query": query})


async def list_airports(search: str = "", limit: int = 10):
    return await call_tool("aviationstack_list_airports", {"search": search, "limit": limit, "offset": 0})


async def list_airlines(search: str = "", limit: int = 10):
    return await call_tool("aviationstack_list_airlines", {"search": search, "limit": limit, "offset": 0})


async def current_weather(city: str):
    return await call_tool("weather_get_current_weather", {"city": city})


async def forecast(city: str):
    return await call_tool("weather_get_forecast", {"city": city})